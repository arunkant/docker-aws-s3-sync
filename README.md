...
## Usage
    ...
    docker run -d [OPTIONS] arunqure/aws-s3-sync
...
* `-e LOCAL_PATH="/data"`: local path to sync (useful when syncing a named volume)
* `- HEALTHCHECK_URL=`: healtcheck urls to ping after each run of cronjob (tested with https://healthchecks.io urls)
...
Sync just once (container is deleted afterwards):
    ...
    docker run --rm \
        ...
        -e BUCKET=mybucket \
        -v /home/user/data:/data:ro \
        arunqure/aws-s3-sync now

## Credits
Forked from [futurevision/docker-aws-s3-sync](https://github.com/futurevision/docker-aws-s3-sync)
